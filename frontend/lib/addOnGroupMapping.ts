...
export type ProductAddonTarget = {
  productId?: string;
  productPath?: string;
  productCode: string;
  productName: string;
  category: string;
  classification: 'FOOD' | 'BEVERAGE' | 'REVIEW';
  existingAddOnGroupIds: string[];
  proposedAddOnGroupIds: string[];
  action: 'ADD' | 'REMOVE' | 'KEEP' | 'REVIEW';
  reason: string;
};
...
export type AddOnOptionDefinition = {
  id: string;
  code: string;
  name: string;
  price: number;
...
export type ProposedAddOnGroupDocument = {
  id: string;
  name: string;
  isActive: true;
  isRequired: false;
...
function createOption(
  ...
  attribute: 'VEG' | 'EGG',
  sortOrder: number,
): AddOnOptionDefinition {
  return { id: code, code, name, price, attribute, isActive: true, sortOrder };
}

export const FOOD_ADD_ON_OPTIONS: AddOnOptionDefinition[] = [
...
export const BEVERAGE_ADD_ON_OPTIONS: AddOnOptionDefinition[] = [
...
export function buildProposedAddOnGroupDocuments(): Record<string, ProposedAddOnGroupDocument> {
  return {
...
const CATEGORY_CONFIRMATION_GAPS = new Map<string, string>(BEVERAGE_EXCLUDED_CATEGORY_ALIASES.map(([alias, approved]) => [normalizeLabel(alias), approved]));
...
export function normalizedCategoryCandidates(product: ProductLike): string[] {
  return [
...
function uniqueStrings(values: unknown): string[] {
  ...
  return Array.from(new Set(array.filter((value): value is string => typeof value === 'string' && value.trim().length > 0).map((value) => value.trim())));
}

export function existingAddOnGroupIds(product: ProductLike): string[] {
  return uniqueStrings(product.addOnGroupIds || product.addonGroupIds);
}

export function classifyProduct(product: ProductLike, categoryNameOverride?: string): 'FOOD' | 'BEVERAGE' | 'REVIEW' {
  const department = normalizeLabel(product.productDepartment || product.department || product.menuType);
  if (department === 'food' || department === 'beverage') return department.toUpperCase() as 'FOOD' | 'BEVERAGE';
...
export function getCategoryApprovalGap(categoryName: string): string | null {
  const normalized = normalizeLabel(categoryName);
  return CATEGORY_CONFIRMATION_GAPS.get(normalized) || null;
}

export function isBeverageExcludedCategory(categoryName: string): boolean {
  const normalized = normalizeLabel(categoryName);
...
export function isRetailCoffeeExempt(product: ProductLike, categoryName?: string): boolean {
...
export function resolveGroupByName(groups: AddOnGroup[], wantedName: string): AddOnGroup | null {
...
export function plannedGroupIds(classification: 'FOOD' | 'BEVERAGE' | 'REVIEW', foodGroupId: string | null, beverageGroupId: string | null): string[] {
  if (classification === 'FOOD') return foodGroupId ? [foodGroupId] : [];
  if (classification === 'BEVERAGE') return beverageGroupId ? [beverageGroupId] : [];
  return [];
}

export function mergeAddonGroupIds(existing: unknown, targetGroupId: string | null): {
  action: 'ADD' | 'REMOVE' | 'KEEP' | 'REVIEW';
  ids: string[];
} {
...
export function removeAddonGroupId(existing: unknown, targetGroupId: string | null): {
  action: 'ADD' | 'REMOVE' | 'KEEP' | 'REVIEW';
  ids: string[];
} {
  const normalizedExisting = uniqueStrings(existing);
...
export function buildAddonAssignment(
  product: ProductLike,
  ...
  const actionResult = classification === 'REVIEW'
    ? { action: 'REVIEW' as const, ids: existingIds }
    : classification === 'FOOD'
