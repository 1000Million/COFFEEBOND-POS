// G4.1: one Admin save must make the POS override and the customer menu snapshot visible
...
const posRow = (item: FinishedGood) => ({ id: item.code, code: item.code, name: item.name, price: item.salePrice, isAvailable: item.isAvailable !== false, isActive: true, sortOrder: item.sortOrder, availableStoreIds: item.availableStoreIds });
const posPrice = (configs: StoreItemConfig[], storeId: string) => resolvePosMenuItems(CATALOGUE.map(posRow), storeId, configs)[0].price;

// ---- PRICE OVERRIDE: one plan carries both writes -------------------------------
...
eq(posPrice(pubPrice.projectedConfigs, GOLDEN), 375, 'A4. POS resolves 375 from the same projected config');
eq(pubPrice.snapshot.menuItems.BOND_FRAPPE.salePrice, posPrice(pubPrice.projectedConfigs, GOLDEN), 'A5. POS and customer prices are equal at publish time — no stale surface');
...
eq(posPrice(pubInherit.projectedConfigs, GOLDEN), 350, 'B4. POS is back to 350 from the same projection');
...
eq(resolvePosMenuItems(CATALOGUE.map(posRow), GOLDEN, pubAvail.projectedConfigs)[0].isAvailable, false, 'C2. POS is unavailable from the same projection');

// ---- CUSTOMER VISIBILITY stays customer-only ---------------------------------------------
...
eq(resolvePosMenuItems(CATALOGUE.map(posRow), GOLDEN, pubVis.projectedConfigs).length, 1, 'D2. POS still carries the item — visibility remains customer-only');
...
eq(posPrice([goldenCfg], NOIDA), 350, 'F2. Noida POS stays on the global 350');
