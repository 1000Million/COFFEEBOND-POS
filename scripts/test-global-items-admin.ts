...
// rounding warning, and route/permission wiring. Pure + static; no Firestore, no network.
import assert from 'node:assert/strict';
import fs from 'node:fs';
import {
  buildOverrideWritePlan, buildResolvedComparison, buildRoundingPreview,
  draftFromConfig, emptyDraft, validateOverrideDraft,
...
ok(page.includes('writeBatch(db)'), 'SNAPSHOT. Page publishes through a single Firestore writeBatch');
ok(page.includes("doc(db, 'publicMenuAvailability', publish.snapshotDocId)"), 'SNAPSHOT2. The customer menu snapshot is written in that same batch');
...
ok(page.includes('/admin/pos-readiness'), 'SNAPSHOT4. POS Readiness remains linked for operational whole-store rebuilds');
ok(!/Customer menu refresh required/.test(page), 'SNAPSHOT5. The stale manual-refresh instruction is gone');
...
const writes = page.match(/batch\.set\(|batch\.delete\(|setDoc\(|deleteDoc\(|updateDoc\(|addDoc\(/g) || [];
eq(writes.length, 3, 'SAFETY2. Exactly three write call sites: batch.delete + batch.set (override) and batch.set (snapshot)');
...
ok(adminHome.includes('to="/admin/global-items"') && adminHome.includes('Global Items'), 'NAV. Admin dashboard tile links to the new page');
