...
eq(storeItemConfigDocId(GOLDEN, 'FG_A'), 'v1|GOLDEN_I|FG_A', 'Deterministic override document id');
...
ok(storeItemConfigDocId('A__B', 'C') !== storeItemConfigDocId('A', 'B__C'), 'Embedded legacy separators cannot collide');
ok(storeItemConfigDocId('A/B', 'C') !== storeItemConfigDocId('A_B', 'C'), 'Escaped path characters cannot alias safe characters');
assert.throws(() => storeItemConfigDocId('A'.repeat(1_500), 'B'), /too long/, 'Overlong identities fail instead of being truncated into a collision');
eq(storeItemConfigDocPath(GOLDEN, 'FG_A'), `${STORE_ITEM_CONFIG_COLLECTION}/v1|GOLDEN_I|FG_A`, 'Deterministic override document path');
