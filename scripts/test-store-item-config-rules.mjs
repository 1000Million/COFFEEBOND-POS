...
// ---- STORE MANAGER: read per isActiveStaff, no writes -------------------------
await assertSucceeds(getDoc(ref(asUser('manager-uid')))); record('STORE_MANAGER read ALLOWED (isActiveStaff)');
...
// ---- CASHIER: read per isActiveStaff, no writes -------------------------------
await assertSucceeds(getDoc(ref(asUser('cashier-uid')))); record('CASHIER read ALLOWED (isActiveStaff)');
...
// ---- CROSS-STORE READ: documents the actual (catalogue-convention) scope -------
await assertSucceeds(getDoc(ref(asUser('manager-uid'), DOC_B))); record('CROSS-STORE: Manager reads an unassigned store row (matches finishedGoods/storeStock convention)');
await assertSucceeds(getDoc(ref(asUser('cashier-uid'), DOC_B))); record('CROSS-STORE: Cashier reads an unassigned store row (same convention)');
